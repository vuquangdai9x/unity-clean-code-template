...
{
    public class BouncingShooterPlayer : MonoBehaviour
    {
        ...
        public SoundFxToPlay sfxShoot = null;
        public ParticleSystem prefabVfxBulletHit = null;

        public class PlayerShootThread : BouncingShootThreadAbstract
        {
            [Header("Bullet config")]
            public float damage = 5f;
            public float critRatio = 0f;
            ...
            protected override bool HandleOnBulletCollided(BulletHellObjectBase bullet, Collider2D target)
            {
                if (target.TryGetComponent<ITakeDamage>(out ITakeDamage targetTakeDamage))
                {
                    ...
                    if (bullet.extDataFloats[INDEX_DATA_FLOAT_DAMAGE] < 1f || bullet.extDataInts[INDEX_DATA_INT_COUNT_HIT] >= maxCountHitAllows)
                    {
                        return true;
                    }
                    else
            ...
            protected override void OnBulletRemoved(BulletHellObjectBase bullet)
            {
                if (bullet && DaiVQObjectPool.IsExistInstance) DaiVQObjectPool.Instance.PlayParticleOnce(prefabVfxBulletHit, bullet.transform.position, Quaternion.LookRotation(Vector3.forward, bullet.transform.up));
                base.OnBulletRemoved(bullet);
            }
        ...
        public BouncingShootConfig shootConfig = default;
        ...
        public void StartShoot(System.Action<PlayerShootThread> onFinishedShooting = null, System.Action<PlayerShootThread> onAllBulletDisappeared = null)
        {
            ...
            playerShootThread.SetUp(this, (BouncingShootConfig)shootConfig.Clone());
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (shootConfig.isUseWorldRectBoundary)
            {
                Gizmos.DrawWireCube(UIPosToWorld.worldRect.center, new Vector3(UIPosToWorld.worldRect.width, UIPosToWorld.worldRect.height, 0.1f));
            }
            else
            {
                Gizmos.DrawWireCube(shootConfig.customBoundary.center, new Vector3(shootConfig.customBoundary.width, shootConfig.customBoundary.height, 0.1f));
            }
        }
    }
}