...
{
    public abstract class BouncingShootThreadAbstract
    {
        private BouncingBulletController _bulletsController;
        private MonoBehaviour _context;
        private BouncingShootConfig _shootConfig;
        private Coroutine _corShooting;
        private PrefabStackPool<BulletHellObjectBase> _poolBullet;
        private bool _isShooting;

        public void SetUp(MonoBehaviour context, BouncingShootConfig shootConfig)
        {
            _context = context;
            ...
            _bulletsController = new BouncingBulletController();
            _bulletsController.SetUp(maxNumBullet, HandleOnBulletCollided, OnBulletRemoved);

            _bulletsController.localDirectionalForce = _shootConfig.localDirectionalForce;
            _bulletsController.SetBoundary(_shootConfig.isUseWorldRectBoundary ? UIPosToWorld.worldRect : _shootConfig.customBoundary, _shootConfig.speedMulOnCollidedWall);
        }
        ...
        private IEnumerator IEShoot()
        {
            ...
            BouncingBulletController.ShootInfo bulletShootInfo = new BouncingBulletController.ShootInfo
            {
            ...
            for (int waveIndex = 0; waveIndex < _shootConfig.numWaves; waveIndex++)
            {
                ...
                int GetNumBounce(Vector2Int range, int index, int numWaves)
                {
                    if (range.x == range.y)
                    {
                        return range.x;
                    }
                    else if (_shootConfig.numWaves == 1)
                    {
                        return range.x;
                    }
                    else
                    {
                        return Mathf.RoundToInt(Mathf.LerpUnclamped(range.x, range.y, (float)index / (float)(numWaves - 1.0f)));
                    }
                }
                bulletShootInfo.numBounceHorizontalRemain = GetNumBounce(_shootConfig.numBounceHorizontalRange, waveIndex, _shootConfig.numWaves);
                bulletShootInfo.numBounceVerticleRemain = GetNumBounce(_shootConfig.numBounceVerticalRange, waveIndex, _shootConfig.numWaves);
                bulletShootInfo.numBounceBothRemain = GetNumBounce(_shootConfig.numBounceBothRange, waveIndex, _shootConfig.numWaves);
